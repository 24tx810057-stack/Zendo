...
public class OrderManager {
    ...
    public void updateOrderStatus(String orderId, String newStatus, OnActionCompleteListener listener) {
        updateOrderStatus(orderId, newStatus, null, listener);
    }

    public void updateOrderStatus(String orderId, String newStatus, Map<String, Object> extraUpdates, OnActionCompleteListener listener) {
        db.collection("orders").document(orderId).get().addOnSuccessListener(documentSnapshot -> {
            ...
            if ("Yêu cầu hủy".equals(newStatus)) {
                updates.put("cancelTimestamp", new java.util.Date());
            }
    ...
    public void sendNewOrderNotification(Order order) {
        ...
        String amountStr = df.format(order.getTotalAmount()) + "đ";

        Map<String, Object> adminNotif = new HashMap<>();
        adminNotif.put("userEmail", "admin");
        adminNotif.put("title", "Đơn hàng mới chờ duyệt");
        adminNotif.put("message", "Khách hàng " + order.getUserName() + " vừa đặt một đơn hàng mới. Tổng tiền: " + amountStr);
    ...
    private void sendOrderNotification(Order order, String status) {
        ...
        switch (status) {
            ...
            case "Yêu cầu hủy":
                ...
                Map<String, Object> adminCancelNotif = new HashMap<>();
                adminCancelNotif.put("userEmail", "admin");
                adminCancelNotif.put("title", "Yêu cầu hủy đơn hàng");
                adminCancelNotif.put("message", "Khách hàng yêu cầu hủy đơn #" + shortId);
                adminCancelNotif.put("timestamp", System.currentTimeMillis());
                adminCancelNotif.put("read", false);
                adminCancelNotif.put("type", "admin_order");
                adminCancelNotif.put("orderId", order.getId());
                adminCancelNotif.put("date", new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm", java.util.Locale.getDefault()).format(new java.util.Date()));
                db.collection("notifications").add(adminCancelNotif);
                return;
            default:
                return;
        }

        // Thông báo phản hồi cho Admin (Matching screenshot)
        if (!"Yêu cầu hủy".equals(status)) {
            Map<String, Object> adminFeedback = new HashMap<>();
            adminFeedback.put("userEmail", "admin");
            adminFeedback.put("title", "Xác nhận đơn hàng");
            adminFeedback.put("message", "Bạn vừa cập nhật đơn hàng của " + order.getUserName() + " sang trạng thái: " + status);
            adminFeedback.put("timestamp", System.currentTimeMillis());
            adminFeedback.put("read", false);
            adminFeedback.put("type", "admin_order");
            adminFeedback.put("orderId", order.getId());
            adminFeedback.put("date", new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm", java.util.Locale.getDefault()).format(new java.util.Date()));
            db.collection("notifications").add(adminFeedback);
        }
