...
import com.zendo.apps.viewmodels.OrderViewModel;
...
public class OrderDetailActivity extends AppCompatActivity {
    ...
    private void observeOrderChanges() {
        ...
        viewModel.getOrderDetail(order.getId()).observe(this, updatedOrder -> {
    ...
    private void displayProducts(List<CartItem> items) {
        ...
        for (CartItem item : items) {
            ...
            if (itemWarranty != null && !itemWarranty.isEmpty()) {
                ...
                com.google.firebase.firestore.FirebaseFirestore.getInstance().collection("products").document(item.getProductId()).get()
        ...
        if ("admin".equals(userRole)) {
            if ("Yêu cầu hủy".equals(status)) {
                ...
                binding.btnCancelOrder.setOnClickListener(v -> {
                    ...
                    com.google.firebase.firestore.FirebaseFirestore.getInstance().collection("orders").document(order.getId()).update(updates)
            ...
            if ("Chờ xác nhận".equals(status)) {
                ...
                if (order.isReviewed()) {
                    ...
                    com.google.firebase.firestore.FirebaseFirestore.getInstance().collection("reviews").whereEqualTo("orderId", order.getId()).get()
                            .addOnSuccessListener(queryDocumentSnapshots -> {
                                if (!queryDocumentSnapshots.isEmpty()) {
                                    binding.btnMainAction.setText("Tiếp tục mua sắm");
                                    binding.btnMainAction.setOnClickListener(v -> navigateToHome());
                                    com.google.firebase.firestore.FirebaseFirestore.getInstance().collection("orders").document(order.getId()).update("reviewed", true);
                                } else {
    ...
    private void performCancellation(String reason) {
        ...
        orderManager.updateOrderStatus(order.getId(), newStatus, updates, new OrderManager.OnActionCompleteListener() {
            @Override
            public void onSuccess() {
                if ("Đã hủy".equals(newStatus)) {
                    Toast.makeText(OrderDetailActivity.this, "Đã hủy đơn hàng thành công", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(OrderDetailActivity.this, "Đã gửi yêu cầu hủy đơn hàng", Toast.LENGTH_SHORT).show();
                }
            }
